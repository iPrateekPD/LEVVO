    ...
    <section className="bg-gradient-to-r from-[#21092F] via-[#1B0D36] to-[#120D24] border-2 border-synthMagenta/70 rounded-xl p-4 sm:p-5 shadow-[0_0_20px_rgba(255,42,133,0.2)] flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-arcade text-arcadeGold">
          +{XP_VALUES[difficulty]} XP / +{Math.floor(XP_VALUES[difficulty] / 2)} GP INSTANT REWARD
        </span>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row items-center gap-2.5">
        <input
          ...
          placeholder="What did you just accomplish? (e.g. Washed the car, drank water, meditated 5 mins)..."
        ...
        <div className="flex items-center gap-1 shrink-0 w-full sm:w-auto justify-between sm:justify-start">
          {(["Trivial", "Easy", "Medium"] as const).map((d) => (
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg border font-arcade text-[9px] transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 ${
                difficulty === d
          ...
          <button
            ...
            className="arcade-btn px-4 py-2 bg-gradient-to-r from-arcadeGold to-yellow-400 hover:brightness-110 text-arcadeBlack font-arcade text-xs font-bold rounded-lg border border-yellow-200 shadow-[0_3px_0_#9E8200] flex items-center gap-1.5 shrink-0 disabled:opacity-50 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400"
          >
            <Send className="w-3.5 h-3.5" />
            <span>{isSubmitting ? "LOGGING..." : "LOG WIN"}</span>
          </button>
        </div>
      </form>
    </section>
