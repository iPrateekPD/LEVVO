  ...
  return (
    <section className="bg-gradient-to-r from-[#21092F] via-[#1B0D36] to-[#120D24] border-2 border-synthMagenta/70 rounded-xl p-4 sm:p-5 shadow-[0_0_20px_rgba(255,42,133,0.2)] flex flex-col gap-3">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-1 text-center sm:text-left">
        ...
        <span className="text-[9px] sm:text-[10px] font-arcade text-arcadeGold text-center sm:text-right">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
        <input
          ...
          className="flex-1 w-full bg-[#0E0A1E] border border-cabinetBorder focus:border-synthMagenta text-textPrimary px-3.5 py-2 rounded-lg text-xs outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 placeholder:text-textMuted text-center sm:text-left"
        />

        {/* Difficulty Selector and Action */}
        <div className="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto justify-center sm:justify-start">
          {/* Centered Option Tabs */}
          <div className="flex items-center justify-center gap-1.5 w-full sm:w-auto">
            {(["Trivial", "Easy", "Medium"] as const).map((d) => (
              <button
                ...
                className={`flex-1 sm:flex-initial px-3 py-1.5 rounded-lg border font-arcade text-[10px] transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 ${
                  difficulty === d
          ...
          {/* Centered Log Win Action Button */}
          <button
            type="submit"
            disabled={!winText.trim() || isSubmitting}
            className="arcade-btn px-4 py-2 bg-gradient-to-r from-arcadeGold to-yellow-400 hover:brightness-110 text-arcadeBlack font-arcade text-[10px] sm:text-xs font-bold rounded-lg border border-yellow-200 shadow-[0_3px_0_#9E8200] flex items-center justify-center gap-1.5 w-full sm:w-auto shrink-0 disabled:opacity-50 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400"
          >
